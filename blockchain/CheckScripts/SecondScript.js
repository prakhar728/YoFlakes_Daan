const Token = require('../contractsData/Token.json');
const Timelock = artifacts.require("Timelock")
const Governance = require('../contractsData/Governance.json');
...
module.exports = async function (callback) {
    ...
    const tokenContract = new ethers.Contract(TokenAddress.address,Token,executor);
    const amount = web3.utils.toWei('5', 'ether')

    await tokenContract.transfer(voter1,)
    await tokenContract.delegate(voter1, { from: voter1 })
    await tokenContract.delegate(voter2, { from: voter2 })
    await tokenContract.delegate(voter3, { from: voter3 })
    await tokenContract.delegate(voter4, { from: voter4 })
    await tokenContract.delegate(voter5, { from: voter5 })
